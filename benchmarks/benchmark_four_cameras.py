#!/usr/bin/env python3
"""Latency vs. context length, replicated for 4 camera streams instead of the
usual 2 — the same three π0.5 comparisons as benchmark_two_cameras.py
(Base Single-Frame, Naive Video, Ours/Keyframe-Caching).

Camera count and keyframe count are orthogonal bottlenecks: caching only
avoids *re-encoding stale frames*, it does nothing for the cost of encoding
more *current* cameras every step — both naive and cached pay that in full.
This script isolates that second axis on its own figure rather than folding
two more lines into the main one, where mixing them risks implying caching
helps with camera fan-out too.

No real xarm rig has 4 cameras — PI0/PI05 only wires up base_0_rgb and
left_wrist_0_rgb (xarm_policy.py:60-63). This is a synthetic random-init
benchmark like the other scripts here, with two more camera keys added to
the observation dict. Nothing else needs to change to support that:
embed_prefix (pi0.py) loops over obs.images generically regardless of count,
and Gemma uses RoPE (src/openpi/models/gemma.py — no fixed
position-embedding table), so a longer image-token prefix just costs more
compute, not a hard sequence-length ceiling.

Model hyperparameters otherwise match xarm_mem8_infer (config.py:1428) exactly,
video_encoder=True throughout — num_frames=1 must still go through the
VideoEncoder family, not video_encoder=False, or it measures an unrelated
scan-vs-unroll slowdown. Same server_timing.infer_ms measurement via a real
Policy + WebsocketPolicyServer + WebsocketClientPolicy, same per-point
subprocess isolation, and the same base/naive/cached structure, as
benchmark_two_cameras.py — see that file for the full rationale.
...
_CAMERAS = ["base_0_rgb", "base_1_rgb", "left_wrist_0_rgb", "right_wrist_0_rgb"]
_H, _W = 224, 224
_N_WARMUP = 10
_N_TIMED = 500
_BASE_PORT = 18820
...
def _run_worker(spec: dict) -> dict:
    ...
    cfg = Pi0Config(
        ...
        # video_encoder/num_frames (swept) and camera count (4, not 2).
        pi05=True,
        action_dim=32,
        action_horizon=50,
    ...
    if mode == "cached":
        ...
        # pays the XLA compile — same as benchmark_two_cameras.py.
        # Discard from steady-state timing.
