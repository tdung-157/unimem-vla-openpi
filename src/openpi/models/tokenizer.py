    ...
    def tokenize(
        ...
        if state is not None:
            ...
            if events is not None:
                full_prompt = f"Task: {cleaned_text}, {events}, State: {state_str};\nAction: "
                # event history will be text describing every full completed event
                # so if we are in the tapping left event
                # " grabbed pudding, dropped left, retracted"
                # if we are in the grab pudding event:
                # " "
            else:
