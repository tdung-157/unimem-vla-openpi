...
# Every _full config is shaped for the 2x H100 node as written:
#   *_event_lora     batch 32, 1 GPU   (LoRA; also fits a single 32 GB card)
#   *_event_full     batch 32, 2 GPUs  -> 16 samples x  3 images per device
#   *_keyframe_lora  batch  8, 1 GPU
#   *_keyframe_full  batch 16, 2 GPUs  ->  8 samples x 12 images per device
#   *_video_full     batch 16, 2 GPUs  ->  8 samples x 18 images per device
...
uv run python custom_unimem/train.py "${CONFIG:-pi05_astribot_unimem_keyframe_full}" \
